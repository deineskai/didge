...
def create_household(
    ...
    household.members.append(
        models.HouseholdMember(
            household_id=household.id, user_id=user_id, is_admin=True
        )
    )
...
def delete_household(
    ...
    household = db.query(models.Household).filter(models.Household.id == id).first()

    if not household:
        raise HTTPException(status_code=404, detail="Specified houshold does not exist")
    ...
    is_admin = (
        ...
        .filter(
            ...
            models.HouseholdMember.is_admin,
        )
        .first()
    )
...
@app.get("/households")
def get_households(
    user_id: int = Depends(auth.get_current_user_id), db: Session = Depends(get_db)
):
    households = (
...
def send_household_invitation(
    ...
    if already_member:
        raise HTTPException(
            status_code=400, detail="User already is a member of the household"
        )

    invitation = models.Invitation(household_id=household_id, to_user_id=to_user_id)
    db.add(invitation)
    db.commit()
...
def get_household_invitations(
    ...
    results = (
        db.query(models.Invitation, models.Household.name)
        .join(models.Household, models.Invitation.household_id == models.Household.id)
        .filter(models.Invitation.to_user_id == user_id)
        .all()
    )
...
def respond_to_household_invitation(
    ...
    if accept:
        ...
        if not already_member:
            member = models.HouseholdMember(
                household_id=invitation.household_id, user_id=user_id, is_admin=False
            )
            db.add(member)
    ...
    db.commit()
    return {
        "message": f"Household invitation {'accepted' if accept else 'rejected'} successfully"
    }
...
def remove_household_member(
    ...
    if not member:
        raise HTTPException(
            status_code=404, detail="User is not a member of the household"
        )
...
def get_household(
    ...
    if not sender_is_member:
        raise HTTPException(
            status_code=403, detail="You are not a member of this household"
        )
    household = db.query(models.Household).filter(models.Household.id == id).first()
