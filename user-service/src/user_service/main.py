...
from user_service import models, auth, database
...
def send_friend_request(
    ...
    friend_request = models.FriendRequest(
        from_user_id=current_user.id, to_user_id=to_user.id, status="pending"
    )
...
def respond_to_friend_request(
    ...
    db.commit()
    return {
        "message": f"Friend request {'accepted' if accept else 'rejected'} successfully"
    }
...
def remove_friend(
    ...
    friend_request = (
        ...
        .filter(
            (
                (models.FriendRequest.from_user_id == current_user.id)
                & (models.FriendRequest.to_user_id == friend_user_id)
            )
            | (
                (models.FriendRequest.from_user_id == friend_user_id)
                & (models.FriendRequest.to_user_id == current_user.id)
            )
        )
        .first()
...
def get_friend_requests(
    ...
    requests = (
        db.query(models.FriendRequest)
        .filter(models.FriendRequest.to_user_id == current_user.id)
    ...
    for r in requests:
        from_user = (
            db.query(models.User).filter(models.User.id == r.from_user_id).first()
        )
        result.append(
            {
                "id": r.id,
...
def get_outgoing_friend_requests(
    ...
    requests = (
        db.query(models.FriendRequest)
        .filter(models.FriendRequest.from_user_id == current_user.id)
        .filter(models.FriendRequest.status == "pending")
        .all()
    )
    result = []
